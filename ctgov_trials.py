...
def flatten_json(obj: Any, prefix: str = "", sep: str = ".") -> Dict[str, str]:
    """Recursively flatten a nested dict/list into dotted-key strings."""
    out: Dict[str, str] = {}
    if isinstance(obj, dict):
        for k, v in obj.items():
            new_key = f"{prefix}{sep}{k}" if prefix else k
            out.update(flatten_json(v, new_key, sep))
...
def _extract_findings(study: Dict[str, Any]) -> str:
    ...
    for og in (_s(rs, "outcomeMeasuresModule", "outcomeMeasures", default=[]) or []):
        ...
        for cls in (og.get("classes") or [])[:3]:
            for cat in (cls.get("categories") or [])[:3]:
                for m in (cat.get("measurements") or [])[:6]:
                    v  = clean(m.get("value"))
                    lo = clean(m.get("lowerLimit"))
                    hi = clean(m.get("upperLimit"))
                    if v:
        ...
        for an in (og.get("analyses") or [])[:3]:
            ...
            ci_lo = clean(an.get("ciLowerLimit"))
            ci_hi = clean(an.get("ciUpperLimit"))
            bits  = []
            if pt or pm:
