...
from .utils import (
    ...
    international_alias_batch as _INTL_ALIAS_BATCH_SIZE,
)
...
def _safe_trial_size(val) -> str:
    """Convert a trial enrollment value to a clean string.
    
    Returns 'N/A' for missing, zero, or unparseable values.
    A real enrollment of 0 is not meaningful, so treat it as missing.
    """
    if val is None:
        return "N/A"
    s = str(val).strip()
    if not s or s.lower() in ("0", "0.0", "none", "null", "n/a", "na", ""):
        return "N/A"
    # Try to parse as number to verify it's real
    try:
        n = float(s.replace(",", ""))
        if n <= 0:
            return "N/A"
        # Return as clean integer string
        return str(int(n))
...
def _clean_country_name(name: str) -> str:
    """Remove trailing ':N' suffixes and reject invalid country names.
    ...
    if not name:
        return name
    cleaned = _COUNTRY_COLON_SUFFIX.sub('', name).strip()
    # Reject purely numeric values, single characters, or common junk
    if not cleaned:
        return ""
    if cleaned.replace(".", "").replace("-", "").isdigit():
        return ""
    if cleaned.lower() in ("0", "n/a", "na", "none", "null", "unknown", "not available", ""):
        return ""
    return cleaned


def _normalise_location(loc_str: str) -> str:
    """Clean a location string: strip :N suffixes, reject junk values."""
    if not loc_str or not loc_str.strip():
        return ""
    # Quick check for common junk values
    s = loc_str.strip()
    if s.lower() in ("0", "n/a", "na", "none", "null", "unknown", "not available"):
        return ""
...
def _normalise_row(row: Dict[str, str]) -> None:
    ...
    # Safety: coerce all values to str (Gemini may return ints for trial_size etc.)
    ...
    # Trial size: convert 0 or empty to N/A
    row["trial_size"] = _safe_trial_size(row.get("trial_size", ""))
...
def map_ctgov(raw: Dict[str, Any], molecule: str,
    ...
    row["trial_size"]          = _safe_trial_size(raw.get("actual_enrollment") or raw.get("target_enrollment", ""))
...
def map_ctis(raw: Dict[str, Any], molecule: str,
    ...
    row["trial_size"]            = _safe_trial_size(raw.get("planned_subjects_worldwide", "")
                                    or raw.get("enrolled", ""))
    primary, secondary = _split_locations(raw.get("countries", ""))
    row["trial_location"]        = primary
    row["secondary_locations"]   = secondary
...
def map_eudract(raw: Dict[str, Any], molecule: str) -> Dict[str, str]:
    ...
    row["trial_size"]            = _safe_trial_size(raw.get("results_subjects_worldwide", "")
...
def map_ctri(raw: Dict[str, Any], molecule: str) -> Dict[str, str]:
    ...
    row["trial_study"]           = raw.get("study_type", "") + (
                                      f" | {raw.get('study_design', '')}"
                                      if raw.get("study_design") else "")
    row["trial_size"]            = _safe_trial_size(raw.get("actual_enrollment", "")
...
async def _fetch_metadata_batch(
    ...
    async with semaphore:
        ...
        try:
            ...
            # Always use Google Search grounding — retry up to _META_MAX_RETRIES times
            ...
            for attempt in range(1, _META_MAX_RETRIES + 1):
                ...
                if attempt < _META_MAX_RETRIES:
                    print(f"  [META] Batch {batch_idx+1}: could not parse response "
                          f"(attempt {attempt}/{_META_MAX_RETRIES}), retrying with search grounding …",
                          file=sys.stderr)
                    await _aio.sleep(1.0 * attempt)  # brief backoff between retries

            # All retries exhausted — return empty (fields will be N/A)
            print(f"  [META] Batch {batch_idx+1}: all {_META_MAX_RETRIES} attempts failed to parse. "
                  f"Fields will remain N/A.", file=sys.stderr)
            if raw:
                print(f"  [META] Batch {batch_idx+1}: last raw (first 300 chars): "
    ...
    for row in targets:
        ...
        for field in META_FIELDS:
            ...
            if val and str(val).strip() and str(val).strip().lower() not in ("n/a", "null", "none"):
                existing_val = str(row.get(field, "") or "").strip()
                if not existing_val:
...
def fetch_all(
        ...
        try:
            ...
            for bq_row in result:
                ...
                for single_id in tid.split(","):
                    ...
                    row["trial_size"]            = _safe_trial_size(str(bq_row.Total_Count or ""))
    ...
    with ThreadPoolExecutor(max_workers=1, thread_name_prefix="src") as src_pool:
        futures = {
    ...
    # ══════════════════════════════════════════════════════════════════════
    # PHASE B: Run alias terms.
    #
    # Strategy:
    #   - API sources (CTGOV, CTIS, EudraCT, CTRI, PubMed, WHO, GD):
    #     Each API only accepts one search term at a time, so we call each
    #     API source once per alias, all in parallel.
    #   - INTL (international registries):
    #     Batches aliases up to _INTL_ALIAS_BATCH_SIZE (default 10) per call.
    #     Each call searches 6 registries batched at trial_registries_per_call
    #     (default 3 → 2 Gemini calls per alias batch).
    #   - Other Gemini sources (NICE, TRADE&CONF, INNOV):
    #     Send ALL aliases combined in a SINGLE call per source.
    # ══════════════════════════════════════════════════════════════════════

    if alias_terms:
        print(f"\n--- ALIASES: {alias_terms} ---", file=sys.stderr)

        alias_rows: List[Dict[str, str]] = []

        # API sources: called once per alias, all in parallel
        # NOTE: WHO ICTRP is excluded — it runs only for the primary drug
        # term (too slow for alias expansion, and most WHO results are
        # duplicates of trials already found via other registries).
        api_sources = [
            ("CTGOV",   _src_ctgov),
            ("CTIS",    _src_ctis),
            ("EudraCT", _src_eudract),
        ...
        # Gemini sources (excluding INTL): send ALL aliases in ONE call
        gemini_sources = [
            ("NICE",       _src_nice),
            ("TRADE&CONF", _src_trade_conf),
            ("INNOV",      _src_innovator),
        ...
        # Build a single combined search term for NICE/TRADE/INNOV
        all_aliases_combined = " OR ".join(alias_terms)
        print(f"  [GEMINI SOURCES] Combined alias term: {all_aliases_combined[:120]}…",
              file=sys.stderr)

        # INTL: batch aliases separately (configurable via INTERNATIONAL_ALIAS_BATCH)
        intl_alias_batches = [alias_terms[i:i+_INTL_ALIAS_BATCH_SIZE]
                              for i in range(0, len(alias_terms), _INTL_ALIAS_BATCH_SIZE)]
        print(f"  [INTL] {len(alias_terms)} alias(es) in "
              f"{len(intl_alias_batches)} batch(es) of up to {_INTL_ALIAS_BATCH_SIZE}",
              file=sys.stderr)

        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="alias") as alias_pool:
            futures = {}
            ...
            # INTL: one call per alias batch (up to _INTL_ALIAS_BATCH_SIZE aliases combined)
            for batch_idx, intl_batch in enumerate(intl_alias_batches, 1):
                combined_intl_term = " OR ".join(intl_batch)
                future = alias_pool.submit(_src_intl, combined_intl_term, alias_rows, False)
                futures[future] = f"INTL(alias-batch-{batch_idx})"

            # NICE, TRADE&CONF, INNOV: ONE call each with ALL aliases combined
            for src_name, src_fn in gemini_sources:
                future = alias_pool.submit(src_fn, all_aliases_combined, alias_rows, False)
                futures[future] = f"{src_name}(all-aliases)"
    ...
    # ── Stamp alias_names on every row ────────────────────────────────────────
    alias_names_str = ", ".join(unique_terms)
    for row in unified:
        row["alias_names"] = alias_names_str

    # ── LLM Confidence Check — verify each trial belongs to this drug ─────────
    # Run BEFORE enrichment so fetcher.py can skip low-confidence rows.
    try:
    ...
    # NOTE: Metadata + location backfill is NOT done here.
    # It is handled in fetcher.py's combined enrichment call (Step 2),
    # which extracts metadata + locations + endpoints in a SINGLE Gemini
    # call per batch. This avoids duplicate metadata fetching.
